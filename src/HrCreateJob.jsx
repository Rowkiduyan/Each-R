        ...
        <div className="bg-white rounded-lg shadow p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input className="w-full border rounded px-3 py-2" value={form.title} onChange={(e)=>setField("title", e.target.value)} placeholder="Delivery Driver"/>
            </div>
            <div>
              <label className="block text-sm font-medium mb-1">Depot</label>
              <input className="w-full border rounded px-3 py-2" value={form.depot} onChange={(e)=>setField("depot", e.target.value)} placeholder="Pasig Depot"/>
            </div>
          </div>

          <div>
            <label className="block text-sm font-medium mb-1">Short Description</label>
            <textarea className="w-full border rounded px-3 py-2" rows={3} value={form.description} onChange={(e)=>setField("description", e.target.value)} placeholder="We are seeking a reliable and safety-conscious Truck Driver..."/>
          </div>
            ...
            <div className="space-y-2">
              {form.responsibilities.map((r, i) => (
                <div key={i} className="flex items-center gap-2">
                  <input className="flex-1 border rounded px-3 py-2" value={r} onChange={(e)=>setResp(i, e.target.value)} placeholder="e.g., Safely operate company-based trucks"/>
          ...
          <div className="flex justify-end gap-3 pt-4 border-t">
            ...
            <button
              onClick={() => {
                // In a real app, save to backend. Here, redirect to Applicant Landing with state.
                navigate("/applicantl/home", { state: { newJob: form } });
              }}
